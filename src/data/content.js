export const content = {
  ru: {
    ...
    about: {
      ...
      title: '2+ года в маркетинге с фокусом на воронку и качество спроса',
      subtitle:
        'Проекты в США (Real Estate, EdTech, Healthcare B2B) и России. Свободно работаю на русском и английском (C1).',
      photoAlt: 'Станислав Бегун',
      paragraphs: [
        'На задачу смотрю не только через объём трафика. Важно понять, что человек считал из сообщения, где интерес начинает разваливаться и почему заявка не доходит дальше. В проектах это давало снижение CPL в 2–8 раз за счёт сегментации, перераспределения бюджета и A/B-тестов — без просадки по качеству лидов.',
        ...
        'Медийных метрик обычно недостаточно: важно видеть, где отваливается мотивированная аудитория, где обещание не совпадает с ожиданием и на каком этапе рвётся путь клиента. Поэтому держу вместе performance, CRM и логику сегментов.',
      ...
      focus: [
        {
          title: 'Как работаю',
          text: 'Сначала проверяю, где именно теряется спрос: в сообщении, в квалификации или между лидом и продажей. Потом формулирую гипотезы по сегментам и касаниям, фиксирую критерии успеха и запускаю тесты. Дальше в работе оставляю только решения, которые подтверждаются цифрами; измерения держу прозрачными (UTM, когорты, регулярный разбор).'
        },
        {
          title: 'Дополнительно',
      ...
      usefulIf: {
        ...
        items: [
          'Нужно пересобрать позиционирование и сообщения, чтобы приходили более релевантные заявки.',
          'Нужно снизить CPL/CAC без просадки по качеству лидов и доводить больше заявок до продажи.',
          'Нужны CRM-цепочки (email + SMS), сегментация базы и рабочая lifecycle-логика для B2B с длинным циклом сделки.',
          'Важно быстро проверять гипотезы и не тратить ресурс на то, что не влияет на экономику.'
        ]
      }
    },
    cases: {
      ...
      title: 'Кейсы',
      subtitle: 'Крупные проекты, где приходилось разбираться, что именно ломается в сообщении, доходимости и экономике.',
      caseCta: 'Читать кейс →',
      ...
      items: [

        {
          ...
          period: '2024–2025',
          summary: 'Пересобрал медиамикс и аудитории: CPL снизился в 8 раз, объём лидов вырос.',
          ...
          details: {
            ...
            situation:
              'Zillow уступала Redfin и Trulia по эффективности привлечения. В Meta Ads работали 6 каналов, но часть из них перегревала аудиторию: CPC доходил до $1,92–$1,96, частота — до 27 показов в неделю.',
            ...
            action:
              'Провёл диагностику по CPC, CTR, CPL и Frequency на уровне каналов и сегментов. Разделил аудитории по стадиям CJM, демографии и поведению, отключил 2 неэффективных канала, 70% бюджета перевёл в рабочие связки и 30% оставил на тест новых аудиторий. Добавил frequency cap ≤5 показов в неделю и еженедельные корректировки.',
            experiments: {
              type: 'table',
            ...
            result:
              'CPL снизился до $0,62 (на 87,6% лучше цели $5), ROAS составил 5,8x. Получили 331 248 лидов и выручку $1 188 541. Параллельно улучшились поведенческие сигналы: CTR 1,68% → 2,14%, Frequency 8,7 → 3,13.',
            unitEconomics: {
              type: 'table',
            ...
            learnings:
              'Сильнее всего сработали быстрые отключения неэффективных каналов и контроль частоты показов — отклик стал заметно чище. Самое сложное — синхронизация с медиабайингом. Если бы повторял проект, раньше проверил бы видео-форматы.',
            transfer:
        ...
        },
        {
          ...
          period: '2024–2025',
          summary: 'Улучшил retention на 35%: сработали сегментация и смена формата коммуникации при CPL ниже цели.',
          ...
          details: {
            ...
            situation:
              'Рынок онлайн-образования США был перегрет: CPL держался около целевого $10, но до завершения программы доходило только 2,5–3% студентов.',
            task:
            ...
            action:
              'Разложил путь пользователя от креатива до завершения программы и посмотрел, на каком этапе теряется мотивация. Сегментировал аудиторию по демографии, мотивации и поведению. По тестам стало видно: профессионалы 30–50 лучше реагируют на конкретную карьерную выгоду, чем на общие обещания.',
            experiments: {
              type: 'table',
              headers: ['Эксперимент', 'Сравнение', 'Результат'],
              rows: [
            ...
            result:
              'CPL снизился до $7 (на 30% ниже цели $10), ROAS — 3x. Получили 32 107 лидов и 270 продаж. Retention вырос до 3,39%, что дало улучшение на 35% (расчёт от оплат).',
            unitEconomics: {
              type: 'table',
              headers: ['Показатель', 'Формула', 'Значение'],
              rows: [
            ...
            learnings:
              'Лучше всего сработала двухэтапная связка video → static и конкретный оффер про карьерный рост. Ограничение проекта — длинный цикл решения (30–60 дней), поэтому тесты шли дольше обычного. В похожей задаче раньше закрепил бы retention как ключевую метрику качества.',
            transfer:
        ...
        },
        {
          ...
          period: '2024–2025',
          summary: 'Сместил фокус в экономику покупки и получил CPL вдвое ниже целевого в B2B.',
          ...
          details: {
            ...
            situation:
              'Dental America продавала стоматологическое оборудование владельцам клиник и главным врачам (B2B). Рынок говорил в основном про спецификации и сертификаты, при этом ценовое преимущество компании (на 15–25% ниже рынка) в коммуникации использовалось слабо.',
            task: 'Привлечь высококачественных B2B-лидов при CPL ≤ $100 в условиях длинного цикла продажи 90–180 дней.',
            action:
              'Проанализировал кампании, сообщения конкурентов и результаты тестов. Сегментировал аудиторию на одиночные клиники, сети и дистрибьюторов. Через интервью с продажами и разбор звонков уточнил ключевой запрос ЦА: «Как получить качественное оборудование и не переплатить?»',
            experiments: {
              type: 'table',
              headers: ['Эксперимент', 'Сравнение', 'Результат'],
              rows: [
            ...
            result:
              'CPL составил $48,64 (на 51,4% ниже цели $100), ROAS — 3,4x. Привлекли 3 936 лидов и выручку $651 032. Качество лидов проверяли в еженедельной связке с отделом продаж.',
            unitEconomics: {
              type: 'table',
              headers: ['Показатель', 'Формула', 'Значение'],
              rows: [
            ...
            learnings:
              'Лучше всего сработало сообщение про экономику закупки и регулярная обратная связь от продаж. Сложность — согласование изменений внутри команды. Если повторять проект, раньше добавил бы ROI-калькулятор на лендинг.',
            transfer:
        ...
        },
        {
          ...
          summary: 'Собрал CRM-систему для B2B-логистики: сегментация базы, email/SMS и реактивация.',
          ...
          details: {
            ...
            situation:
              'ПГК — крупнейший частный оператор ж/д перевозок в России. В B2B-базе 500+ клиентов коммуникация шла нерегулярно: без сегментации, без автоворонок и с ограниченной управляемостью повторных касаний.',
            ...
            action:
              'Сегментировал 500+ клиентов на 8 групп по объёму перевозок, типу груза, региону и частоте обращений, отдельно выделил 120 «спящих» клиентов. Настроил календарь касаний, запустил 12+ email, 8+ SMS и онбординг-воронку в Битрикс24 (7 писем за 30 дней).',
            experiments: {
              type: 'table',
              headers: ['Эксперимент', 'Сравнение', 'Результат'],
              rows: [
            ...
            result:
              'Open Rate достиг 24% (на 8% выше среднего по B2B — 16%). Запущено 12+ email и 8+ SMS, в онбординг переведено 15+ новых корпоративных клиентов. Реактивировано 22% «спящих» (26 из 120), повторные обращения выросли примерно на 18%, звонки в поддержку снизились на 39% (оценка).',
            unitEconomics: {
              type: 'table',
            ...
            learnings:
              'Лучше всего сработали сегментация базы, персонализация по типу груза и региону, а также SMS по статусу. Ограничение — формат стажёрской роли. Если повторять, раньше связал бы CRM с системой отслеживания грузов для автоматических триггеров.',
    ...
    resume: {
      ...
      description:
        'Фокус — на качестве спроса и управляемой воронке. Ключевые результаты: CPL ниже в 8 раз (Zillow), retention выше на 35% в EdTech-проекте (Capella), B2B-лиды с CPL вдвое ниже цели (Dental America), запуск CRM-системы 12+ email и 8+ SMS с OR 24% (ПГК). Опыт: K.Target — Digital-маркетолог / Marketing Analytics Specialist (Май 2024 – Дек 2025), проекты из США: Real Estate, EdTech, Healthcare B2B, SaaS, бюджет $621K, выручка $2,5M+. Первая Грузовая Компания — Digital-маркетолог / CRM-менеджер (Дек 2023 – Апр 2024), CRM-коммуникации для B2B-логистики, сегментация 500+ клиентов. Навыки: аналитика, performance, CRM, unit-экономика, SEO. Языки: русский (родной), английский (C1).',
    ...
    contacts: {
      ...
      title: 'На связи',
      subtitle:
        'Рассматриваю роли в продуктовых и growth-командах. Не работаю с проектами в индустриях: gambling, оружие, товары двойного назначения с военным использованием, алкоголь и табак.',
      items: [
