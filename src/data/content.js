      ...
      items: [
        ...
        },
        {
          ...
          details: {
            ...
            result:
              'CPL снизился до $7 (на 30% ниже цели $10), ROAS — 3x. Получили 32 107 лидов и 270 продаж. Retention вырос до 3,39%, что дало улучшение на 35%. Кейс подтвердил, что не весь трафик одинаково полезен: часть эффекта дала именно работа с намерением и сообщением.',
            unitEconomics: {
              type: 'table',
              headers: ['Показатель', 'Что показывает', 'Значение'],
              rows: [
            ...
            learnings:
              'Лучше всего сработала двухэтапная связка video → static и конкретный оффер про карьерный рост. Ограничение проекта — длинный цикл решения, поэтому тесты шли дольше обычного. В похожей задаче раньше закрепил бы retention как ключевую метрику качества и принимал бы по нему решения быстрее.',
            transfer:
              'В Метрике создаю динамические аудитории (смотрел видео, но не оставил заявку) → экспорт в VK Рекламу для статичных офферов. Цепочки: видео → статика → персонализированный оффер. Когорты по неделям для отслеживания retention.'
          }
        },
        {
          id: 'dental-america',
        ...
        },
        {
          ...
          details: {
            ...
            task:
              'Запустить систему CRM-коммуникаций email + SMS, сегментировать базу, создать автоворонки для онбординга и повысить retention.',
            action:
              'Сегментировал 500+ клиентов на 8 групп по объёму перевозок, типу груза, региону и частоте обращений, отдельно выделил 120 «спящих» клиентов. Настроил календарь касаний и связал его с lifecycle-логикой по сегментам. Запустил 12+ email, 8+ SMS и онбординг-воронку в Битрикс24.',
            experiments: {
              type: 'table',
              headers: ['Эксперимент', 'Сравнение', 'Результат'],
              rows: [
            ...
            result:
              'Open Rate достиг 24% (на 8% выше среднего по B2B). Запущено 12+ email и 8+ SMS, в онбординг переведено 15+ новых корпоративных клиентов. Реактивировано 22% «спящих» клиентов, повторные обращения выросли примерно на 18%, звонки в поддержку снизились на 39% (оценка). Для junior-контекста это показало, что даже базовая CRM-дисциплина заметно улучшает удержание и сервисную нагрузку.',
            unitEconomics: {
              type: 'table',
              headers: ['Показатель', 'Что показывает', 'Значение'],
              rows: [
